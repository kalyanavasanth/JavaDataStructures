...
public class FindingLoop<T1> {
	 ...
	 * @param _list
	 *			  passes the reference of the list in which loop has to be created
	 */
	public void CreateLoopAt(InsecureLinkedListHelper<T1> _list, int position) {
		list = _list;
		InsecureLinkedListHelper<T1>.Node IndexPointer=list.getHead();  //get access to head element
		InsecureLinkedListHelper<T1>.Node EndPointer=IndexPointer;
		for (int current_pos = 0; current_pos < position; ++current_pos) {
				IndexPointer=IndexPointer.getNext();  //traverse IndexPointer to specified index
		}
		while(EndPointer.getNext()!=null){
			EndPointer=EndPointer.getNext();  //traverse EndPointer till end
		}
		EndPointer.setNode(IndexPointer); //Set next of EndPointer to IndexPointer to create the loop
	}
